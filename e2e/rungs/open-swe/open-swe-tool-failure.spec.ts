...
async function streamTurn(page: Page, frames: object[]) {
  await page.route("**/api/chat/stream**", (route) =>
    void route.fulfill({
      status: 200,
      headers: SSE,
      body: frames.map((f) => `data: ${JSON.stringify(f)}`).join("\n\n") + "\n\n",
    })
  );
}
...
test.describe("open-swe chat — a tool that fails is shown as failed", () => {
  ...
  test("A TOOL THAT THREW IS NOT MARKED complete", async ({ page }) => {
    ...
    await streamTurn(page, [
      ...CALL,
      { type: "tool-output-error", toolCallId: "tc-1", errorText: "counter service unavailable" },
      { type: "finish" },
    ]);
    await send(page);
  ...
  test("A SUCCESSFUL TOOL IS STILL MARKED complete", async ({ page }) => {
    ...
    await streamTurn(page, [
      ...CALL,
      { type: "tool-output-available", toolCallId: "tc-1", output: "Counter incremented to 38" },
      { type: "finish" },
    ]);
    await send(page);
  ...
  test("a failed and a successful tool in ONE turn are told apart", async ({
    ...
    await streamTurn(page, [
      ...
      { type: "tool-output-error", toolCallId: "bad-1", errorText: "write refused" },
      { type: "finish" },
    ]);
    await send(page);
    ...
    await expect(
      page.locator('[data-tool-status="complete"]')
    ).toHaveCount(1);
