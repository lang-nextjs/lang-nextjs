...
const logger = createLogger(LogLevel.INFO, "GenExecutingTestsActionsNode");
...
function formatSystemPrompt(
  ...
  // Get testing context from recent testing messages
  const testingContext = state.testingMessages
    .slice(-10)
  ...
  return SYSTEM_PROMPT.replaceAll(
    ...
    .replaceAll(
      "{TESTING_CONTEXT}",
      testingContext || "No previous testing messages",
    )
    .replaceAll(
      "{USER_REQUEST_PROMPT}",
...
export async function genExecutingTestsActions(
  state: TestingAgentState,
  config: GraphConfig,
  ...
  logger.info("Generated executing tests actions", {
