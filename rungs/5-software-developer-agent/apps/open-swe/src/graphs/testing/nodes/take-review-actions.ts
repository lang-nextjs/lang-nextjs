...
const logger = createLogger(LogLevel.INFO, "TakeReviewActions");

export async function takeReviewActions(
  ...
  const textEditorTool = createTextEditorTool(adaptedState, config);
  const allTools = [
    shellTool,
    searchTool,
    viewTool,
  ...
  const toolCallResultsPromise = toolCalls.map(async (toolCall) => {
    ...
    logger.info("Executing test writing action", {
      ...toolCall,
    });
  ...
  logger.info("Completed test writing actions", {
  ...
  // If we've reached the max allowed testing actions, go to executing tests.
  if (filteredMessages.length >= maxActionsCount) {
    logger.info("Exceeded max actions count, going to executing tests.", {
      maxActionsCount,
      filteredMessages,
    });
    return new Command({
      goto: "gen-executing-tests-actions",
