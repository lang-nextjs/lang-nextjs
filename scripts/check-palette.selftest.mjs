...
function check(name, actual, expected) {
  ...
  if (!ok) failures++;
  console.log(`  ${ok ? "OK  " : "FAIL"} ${name}  (findings=${actual}, expected=${expected})`);
}
...
try {
  ...
  check("baseline: the real repo roots are clean", scan(["apps/example", "e2e"]).length, 0);

  // 2. Semantic tokens must NOT be flagged — they are the correct answer.
  fixture("ok-tokens.tsx", `
  ...
  fixture("rare.tsx", `export const A = <div className="text-fuchsia-300 border-lime-700" />;`);
  ...
  fixture("prefix.tsx", `export const A = <div className="text-emerald-400 ring-sky-200" />;`);
