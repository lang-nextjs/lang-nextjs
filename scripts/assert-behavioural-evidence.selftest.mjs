...
const CHECKER = join(
  ...
  "assert-behavioural-evidence.mjs"
);
...
function sandbox(
  ...
  writeFileSync(
    join(pkg, "index.ts"),
    barrel.map((s) => `export { ${s} } from "./x";`).join("\n")
  );
...
function run(dir) {
  try {
    return {
      rc: 0,
      out: execFileSync("node", [CHECKER, "--cwd", dir, "--list"], {
        encoding: "utf8",
      }),
    };
  } catch (e) {
...
expect(
  "a bare surface verb as the whole evidence",
  "reject",
  [
    {
      id: "STR-02",
      desc: "the stream resumes after an interruption without duplication",
      evid: "EXPORTED",
    },
  ],
  {},
...

{
  const dir = sandbox(
    [
      {
        id: "E2E-09",
        desc: "retry() resumes without duplicating",
        evid: "asserts requestCount=2, 'First part' count=1",
      },
    ],
    {
      baseline: {
        ...
        positiveControls: [
          {
            row: ".planning/phases/NOPE-VERIFICATION.md:E2E-99",
            why: "a control that does not exist",
          },
        ],
      },
    }
  ...
  if (rc === 1 && out.includes("NOT PARSED")) {
    console.log(`  ok   ${label.padEnd(62)} (reject)`);
    pass++;
  } else {
...

{
  const dir = sandbox(
    ...
    ],
    {
      baseline: {
        known: [
          {
            row: ".planning/phases/x-VERIFICATION.md:E2E-09",
            issue: "#0",
            why: "already fixed",
          },
        ],
  ...
  if (rc === 1 && out.includes("stale baseline entry")) {
    console.log(`  ok   ${label.padEnd(62)} (reject)`);
    pass++;
  } else {
    console.error(`  FAIL ${label} — rc=${rc}`);
