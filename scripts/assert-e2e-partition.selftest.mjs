...
const CHECKER = join(dirname(fileURLToPath(import.meta.url)), "assert-e2e-partition.mjs");
...
function sandbox({ listing, frozen, brokenBinary = false }) {
  ...
  writeFileSync(bin, brokenBinary
    ? `#!/bin/sh\necho "config error" >&2\nexit 1\n`
    : `#!/bin/sh\ncat <<'JSON'\n${JSON.stringify(listing)}\nJSON\n`);
  chmodSync(bin, 0o755);
  if (frozen) writeFileSync(join(dir, "scripts", "e2e-partition.json"), JSON.stringify(frozen));
  return dir;
}
...
const listingOf = (pairs) => ({
  suites: pairs.map(([project, file]) => ({
    file, specs: [{ file, tests: [{ projectName: project }] }], suites: [],
  })),
});
...
const PAIRS = [["chromium", "e2e/a.spec.ts"], ["webkit", "e2e/a.spec.ts"], ["shell", "e2e/b.spec.ts"]];
const FROZEN = PAIRS.map(([p, f]) => `${p}\t${f}`).sort();

console.log("\nassert-e2e-partition — ACCEPT\n");

expect("what runs matches what is declared", "accept",
 ...
 */
expect("a spec that JOINED a project it was not declared in", "reject",
  { listing: listingOf([...PAIRS, ["chromium", "e2e/b.spec.ts"]]), frozen: FROZEN },
 ...
 */
expect("a spec that LEFT a project it is declared in", "reject",
  { listing: listingOf(PAIRS.filter(([p]) => p !== "webkit")), frozen: FROZEN },
  ["LEFT project \"webkit\"", "0 joined, 1 left"]);

expect("both directions at once are reported separately", "reject",
  { listing: listingOf([["chromium", "e2e/a.spec.ts"], ["shell", "e2e/b.spec.ts"], ["firefox", "e2e/c.spec.ts"]]), frozen: FROZEN },
  ["JOINED", "LEFT", "1 joined, 1 left"]);

console.log("\nassert-e2e-partition — REFUSALS\n");

expect("no frozen declaration is a refusal, not a pass", "refuse",
  { listing: listingOf(PAIRS), frozen: null }, ["no scripts/e2e-partition.json"]);
 ...
 */
expect("an empty listing is a refusal, not 'no differences'", "refuse",
  { listing: { suites: [] }, frozen: FROZEN }, ["resolved 0 project/spec pairs", "vacuously"]);

expect("a playwright that fails is a refusal, not an intact partition", "refuse",
  { listing: null, frozen: FROZEN, brokenBinary: true }, ["--list failed", "COULD NOT COMPUTE"]);
