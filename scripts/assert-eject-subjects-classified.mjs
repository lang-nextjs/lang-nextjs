...
export function problemGroups(registered, census) {
  ...
  return [
    ...
    },
    {
      ...
      fix:
        `  Fix: EDIT scripts/eject-subject-census.json BY HAND. What this gate READ was a\n` +
        `  "${STATIC}" verdict whose \`note\` is absent or empty, and prose is the one\n` +
        `  thing no run produces.\n` +
        `\n  DO NOT RUN \`pnpm eject-audit\` FOR THIS ONE. The producer writes\n` +
        `  \`note: keep ? old.note : null\` — it CARRIES a note forward and never\n` +
        `  ORIGINATES one, so a note that does not exist cannot be generated by running\n` +
        `  anything. Successive runs return IDENTICAL totals, which reads as convergence\n` +
        `  and is the loop failing to close. That has cost eight minutes twice.\n` +
        `\n  Write why the subject's domain does not vary by rung. If \`lifts\` is the\n` +
        `  complaint instead, it must be null (permanent) or "#NNN" (an open issue).`,
    },
