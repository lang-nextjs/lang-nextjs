...
const verdict = (dir) =>
  readFileSync(join(dir, "sub", "f.txt"), "utf8").includes("bad");
...
// 1. a real mutation that flips the verdict
{
  ...
  quiet(() =>
    r.expect("reject", "flips", (d) =>
      writeFileSync(join(d, "sub", "f.txt"), "bad\n")
    )
  );
  check(
    "a mutation that flips the verdict passes",
    [r.results.ok, r.results.void, r.results.failed],
    [2, 0, 0]
  );
}
...
// 2. THE ROT: a mutation that changes nothing
{
  ...
  check(
    "an INERT mutation is VOID, not FAIL",
    [r.results.void, r.results.failed],
    [1, 0]
  );
}
...
// 3. declared non-mutating, and does not mutate
{
  ...
  check(
    "{ mutates: false } that does not mutate passes",
    [r.results.ok, r.results.void],
    [2, 0]
  );
}
...
// 4. declared non-mutating but DOES mutate — the declaration is a claim too
{
  ...
  quiet(() =>
    r.expect(
      "accept",
      "liar",
      (d) => writeFileSync(join(d, "sub", "f.txt"), "good2\n"),
      { mutates: false }
    )
  );
...
// 5. DEV2's open region: mutates, expected not to flip
{
  const r = createProofRunner({ makeFixture, verdict });
  quiet(() =>
    r.expect("accept", "no flip", (d) =>
      writeFileSync(join(d, "sub", "g.txt"), "x\n")
    )
  );
  check(
    "an accept-case that mutates is counted UNPROVEN",
    [r.results.unproven, r.results.failed],
    [1, 0]
  );
  check(
    "...and still exits 0 (documented gap, not a guard)",
    quiet(() => r.report()),
    0
  );
}
...
// 6. a baseline that already fails voids the run
{
  ...
  quiet(() =>
    r.expect("reject", "would pass for the wrong reason", (d) =>
      writeFileSync(join(d, "sub", "f.txt"), "bad bad\n")
    )
  );
  check("a REJECTED baseline fails the run", [r.results.failed > 0], [true]);
  check(
    "...and report() exits non-zero",
    quiet(() => r.report()),
    1
  );
}
...
// 7. verdict mismatch is a plain FAIL
{
  const r = createProofRunner({ makeFixture, verdict });
  quiet(() =>
    r.expect("reject", "does not flip", (d) =>
      writeFileSync(join(d, "sub", "f.txt"), "still good\n")
    )
  );
  check(
    "a mutation that moves bytes but not the verdict FAILs",
    [r.results.failed],
    [1]
  );
}
...
// 8. VOID must fail the run — a missing proof is not a pass
{
  ...
  quiet(() => r.expect("reject", "inert", () => {}));
  check(
    "report() exits non-zero when anything is VOID",
    quiet(() => r.report()),
    1
  );
}
...
const buildThenRead = (dir) => {
  // an HONEST build
  writeFileSync(
    join(dir, "dist", "a.txt"),
    readFileSync(join(dir, "src", "a.txt"), "utf8")
  );
  return readFileSync(join(dir, "dist", "a.txt"), "utf8").includes("bad");
};
const cachedBuild = (
  dir // a CACHED/skipped build
...
{
  const r = createProofRunner({
    makeFixture: makeCompiled,
    verdict: buildThenRead,
  });
  quiet(() =>
    r.expect("reject", "compiled", mutateSrc, { witness: witnessDist })
  );
  check(
    "witness satisfied by an honest build passes",
    [r.results.ok, r.results.void],
    [2, 0]
  );
}
...
// 12. THE GAP: cached build. Source moved, checker ran, artifact never changed.
{
  ...
  check(
    "a CACHED build is VOID, not FAIL",
    [r.results.void, r.results.failed],
    [1, 0]
  );
}
...
//     which blames the checker for a build that never happened.
{
  ...
  check(
    "...and without a witness it is indistinguishable from a FAIL",
    [r.results.failed],
    [1]
  );
}
...
// 14. a witness already true cannot discriminate
{
  ...
  quiet(() =>
    r.expect("reject", "pre-true witness", mutateSrc, { witness: () => true })
  );
...
// 15. artifactContains tolerates a missing artifact
{
  ...
  check(
    "artifactContains(missing file) is false, not a throw",
    artifactContains(d, "dist/a.txt", "bad"),
    false
  );
  rmSync(d, { recursive: true, force: true });
}
...
if (pass === total) {
  console.log(
    `PASS: ${pass}/${total}. mutation-proof.mjs has been observed emitting every verdict it`
  );
  console.log(
    `      can emit — ok, FAIL, VOID (inert, mis-declared, pre-true witness, stale`
  );
