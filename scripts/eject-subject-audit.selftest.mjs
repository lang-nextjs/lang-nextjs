...
/*
 * 90 base arms + 3 tail probes. A child running a probe skips the probes (so 90) and carries
 * one planted arm, and `fail-counted` bumps by one so the count guard cannot speak over the
 * failure branch — which is the whole point of the pair.
 */
const EXPECTED =
  TAIL_PROBE === "" ? 93 : TAIL_PROBE === "fail-counted" ? 91 : 90; // +6 for #1071's carried ruling, 37 + 6 for #843 + 8 for #855 + 4 for #844's external rule + 5 for #875 + 3 for #876/#883 + 13 for #920 + 4 for #1170's declared exemptionconst total = pass + fail;
/*
