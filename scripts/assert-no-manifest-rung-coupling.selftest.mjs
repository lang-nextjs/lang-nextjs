...
import { stripComments, manifestBindings, couplings } from "./assert-no-manifest-rung-coupling.mjs";
...
const hits = (src) =>
  couplings(stripComments(src), manifestBindings(stripComments(src)), REMOVABLE);

const IMPORT = 'import { RUNG_BY_ID } from "@deepagents-nextjs/rungs";\n';

// ── PURE: the finding ──────────────────────────────────────────────────────
check("REJECT: a shared file indexing a removable rung", () =>
  hits(IMPORT + 'const t = RUNG_BY_ID["open-swe"].target;').length === 1
);

check("REJECT: single quotes and whitespace do not evade it", () =>
  hits(IMPORT + "const t = RUNG_BY_ID [ 'deepagents' ];").length === 1
);

check("REJECT: an aliased import is still tracked", () =>
  hits(
...
check("ACCEPT: the id appears only in a COMMENT", () =>
  hits(IMPORT + '// RUNG_BY_ID["open-swe"] used to be hardcoded here\nconst t = 1;').length === 0
);

check("ACCEPT: the id appears only in a BLOCK comment", () =>
  hits(IMPORT + '/*\n * RUNG_BY_ID["deepagents"] was the old shape.\n */\nconst t = 1;').length === 0
);

check("ACCEPT: the LOWEST rung is not removable, so naming it couples nothing", () =>
  hits(IMPORT + 'const t = RUNG_BY_ID["langchain"];').length === 0
);

check("ACCEPT: an identically-named symbol NOT from the generated package", () =>
  hits('import { RUNG_BY_ID } from "./local-map";\nconst t = RUNG_BY_ID["open-swe"];').length === 0
);

check("ACCEPT: the rung id as a plain string, not an index", () =>
...
check("stripComments leaves string contents alone", () =>
  stripComments('const u = "http://x/a//b"; // trailing').trim() === 'const u = "http://x/a//b";'
);
...
check("LIVE: this repository passes today", () => {
  const r = run();
  return r.code === 0 ? true : `checker rejected a clean tree:\n${r.out.slice(0, 400)}`;
});
...
try {
  ...
  check("LIVE: it FAILS on the instance as #374 found it, naming file and rung", () => {
    const r = run();
    if (r.code === 0) return "the checker passed over the very coupling it exists for";
    if (!r.out.includes(INSTANCE)) return `did not name the file:\n${r.out.slice(0, 300)}`;
    if (!r.out.includes('RUNG_BY_ID["open-swe"]')) return `did not name the reference:\n${r.out.slice(0, 300)}`;
    return true;
  });
} finally {
