...
function fixture(extra = "") {
  ...
  for (let i = 0; i < 12; i++) {
    writeFileSync(join(dir, "src", `a${i}.test.ts`), `it("case ${i}", () => {});\n`);
  }
...
function run(dir) {
  try {
    return { rc: 0, out: execFileSync("node", [SUT, "--cwd", dir], { encoding: "utf8", stdio: ["ignore","pipe","pipe"] }) };
...
const check = (name, want, extra, verify) => {
  ...
  if (okRc && (verify ? verify(out) : true)) { console.log(`  ok   ${name.padEnd(54)} (${want}ed)`); pass++; }
  else { console.error(`  FAIL ${name.padEnd(54)} rc=${rc}`); console.error(`       ${out.split("\n")[0]}`); fail++; }
};

console.log("assert-no-silent-skips self-test — flags what runs nowhere, spares what states a condition\n");

// --- REJECT: unconditional skips, in every form ----------------------------------------------
check("it.skip is flagged", "reject", `it.skip("dead one", () => {});\n`);
check("describe.skip is flagged", "reject", `describe.skip("dead group", () => {});\n`);
...
check("describe.skipIf is NOT flagged", "accept", `describe.skipIf(!process.env.LIVE)("live", () => {});\n`);
check("it.runIf is NOT flagged", "accept", `it.runIf(process.env.LIVE)("live", () => {});\n`);
check("it.skipIf is NOT flagged", "accept", `it.skipIf(!process.env.LIVE)("live", () => {});\n`);
...
check("a comment mentioning it.skip is not flagged", "accept",
  `// we used to it.skip("this") but no longer\n/* describe.skip("nor this") */\nit("real", () => {});\n`);

// --- REJECT: the scan finding nothing must fail, not pass -------------------------------------
{
  const dir = join(TMP, "empty"); mkdirSync(dir, { recursive: true });
  execFileSync("git", ["init", "-q"], { cwd: dir });
  writeFileSync(join(dir, "readme.md"), "no tests here");
  execFileSync("git", ["add", "-A"], { cwd: dir });
  const { rc, out } = run(dir);
  if (rc !== 0 && out.includes("scan is broken")) {
    console.log(`  ok   ${"a tree with no test files fails, not passes".padEnd(54)} (rejected)`); pass++;
  } else { console.error(`  FAIL empty tree (rc=${rc})`); fail++; }
}

// --- ACCEPT: the real repo, which must be clean or the gate is unshippable ---------------------
{
  const { rc } = run(ROOT);
  if (rc === 0) { console.log(`  ok   ${"the real repository passes".padEnd(54)} (accepted)`); pass++; }
  else { console.error(`  FAIL the real repository does not pass`); fail++; }
}
...
if (fail) { console.error(`FAIL: ${fail}/${total} wrong.`); process.exit(1); }
console.log(`PASS: ${pass}/${total}. Unconditional skips are caught in every form, conditional ones`);
console.log(`      are spared, and a scan that found nothing fails rather than reporting clean.`);
