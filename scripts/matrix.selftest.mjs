...
function run(manifest) {
  ...
  try {
    return {
      rc: 0,
      out: execFileSync("node", [SUT, "--github"], {
        encoding: "utf8",
        env: { ...process.env, RUNGS_MANIFEST: p },
...
const check = (name, want, mutate, verify) => {
  ...
  if (okRc && okExtra) {
    console.log(`  ok   ${name.padEnd(52)} (${want}ed)`);
    pass++;
  } else {
...
console.log(
  "matrix.mjs self-test — the arity is derived, and cannot silently be wrong\n"
);
...
check(
  "a 1-rung manifest emits only that rung's jobs",
  "accept",
  (m) => {
    m.rungs = m.rungs.slice(0, 1);
  },
  (out) => {
    const jobs = JSON.parse(out.replace(/^matrix=/, "")).include;
    return (
      jobs.length === REAL.rungs[0].languages.length &&
      jobs.every((j) => j.rung === REAL.rungs[0].id)
    );
  }
);
...
check(
  "zero rungs is refused, not emitted as an empty matrix",
  "reject",
  (m) => {
    m.rungs = [];
  }
);
check("a rung with no languages is refused", "reject", (m) => {
  m.rungs[0].languages = [];
});
...
check(
  "every job carries rung, lang and a retain set",
  "accept",
  null,
  (out) => {
    const jobs = JSON.parse(out.replace(/^matrix=/, "")).include;
...
console.log(
  `PASS: ${pass}/${total}. Arity follows the manifest, a fork emits fewer jobs, and a`
);
console.log(
  `      manifest that would yield an empty matrix is refused rather than emitted.`
);
