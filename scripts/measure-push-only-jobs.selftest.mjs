...
const ok = (label, cond, got) => {
  ...
  else { console.error(`  FAIL ${label}${got !== undefined ? ` — got ${JSON.stringify(got)}` : ""}`); fail++; }
};

// Newest-first, as `gh run list` returns them.
const F = "failure", S = "success", C = "cancelled";

console.log("\nstreaks — how long a red survives\n");

let r = streaks([F, F, F, S, F, S]);
ok("current streak counts only the unbroken run at the newest end", r.current === 3, r);
ok("longest streak is the longest anywhere in the window", r.longest === 3, r);

r = streaks([S, F, F, F, F]);
ok("a green at the newest end means the current streak is 0", r.current === 0, r);
...
ok("a cancelled run neither breaks nor extends a streak", r.current === 5 && r.longest === 5, r);

r = streaks([C, C, C]);
ok("a window with no verdicts at all has no streak", r.longest === 0 && r.current === 0, r);

r = streaks([F, F, F]);
ok("never green in the window is reported as such", r.everGreen === false && r.current === 3, r);

r = streaks([S, S, S]);
ok("all green is zero, not one", r.longest === 0 && r.current === 0 && r.everGreen, r);
...
ok("cancelled runs are excluded from the denominator", s.concluded === 3 && s.seen === 4, s);
ok("the rate is over CONCLUDED runs, not over the sample", Math.abs(s.rate - 2 / 3) < 1e-9, s);
/*
 ...
 */
ok("a cancelled run in the middle does not break the current streak", s.current === 2, s);
