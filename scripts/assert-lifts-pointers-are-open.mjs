/**
 * `scripts/checks.json`'s `unregistered[].lifts` NAMES AN OPEN ISSUE (#824).
 *
 * READ THE SUBJECT LINE ABOVE LITERALLY. There are TWO `lifts` fields in this repo and this
 * checker reads exactly one of them:
 *
 *     scripts/checks.json           unregistered[].lifts    COVERED HERE
 *     scripts/eject-subject-census.json  checkers[].lifts    NOT COVERED BY ANYTHING
 *
 ...
 *
 * SO THE CENSUS FIELD IS UNCOVERED AND IS WHERE THE ONLY LIVE POINTER ACTUALLY LIVES —
 * `eject-subjects-classified` carries `lifts: "#780"` there, while every pointer in the file
 * this check reads is `null`. A reader who sees a check named `lifts-pointers-open` will assume
 * both are guarded. They are not, and an uncovered field a reader believes is covered is worse
 * than one nobody has claimed. It is filed separately rather than bolted on here: the census is
 * REGENERATED by `pnpm eject-audit`, so its repair is "re-run the producer" while this file's is
 * "edit the entry" — same field name, opposite lifecycle, and one verdict must not span both.
 *
...
function main() {
  ...
  reportSubject(
    found.length,
    "`checks.json` `unregistered[].lifts` pointer(s) checked against the board"
  );
  console.log(
    found.length === 0
      ? `PASS: no \`checks.json\` exclusion names an issue in \`lifts\`, so there is no\n` +
          `      pointer HERE to be stale. All ${examined} entr${
            examined === 1 ? "y is" : "ies are"
          } \`null\`, which this check cannot\n` +
          `      verify and does not claim to. That count is read from the file on every run\n` +
          `      rather than written here, because it moved the night this check was authored.\n` +
          `      eject-subject-census.json's \`checkers[].lifts\` is a DIFFERENT field, it holds\n` +
          `      the only live pointer in the repo, and no check covers it — including this one.`
      : `PASS: every \`checks.json\` \`lifts\` pointer names an open issue, out of ${examined} exclusion(s) read.`
  );
}
