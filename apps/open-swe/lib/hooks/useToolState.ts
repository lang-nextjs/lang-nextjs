      ...
      if (event.type === "tool-input-start") {
        ...
        if (existing) {
          ...
          state.set(event.toolCallId, {
            ...
            status: pendingOutputs.has(event.toolCallId)
              ? "completed"
              : existing.status,
          });
        } else {
        ...
        // tool-input-available arrives after tool-input-start and carries the parsed args.
        // If we already have a pending entry, update its input; otherwise create one.
        const existing = state.get(event.toolCallId);
        if (existing) {
          state.set(event.toolCallId, {
            ...existing,
