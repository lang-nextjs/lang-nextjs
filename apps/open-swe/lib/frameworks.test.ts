...
describe("topologiesFor — derived from the manifest, not restated", () => {
  ...
  it("pins the whole (rung, runtime) grid as a literal", () => {
    ...
    //
    // THIS CASE IS ALSO THE TRIPWIRE FOR THE DISTRIBUTION ITSELF. Two separate
    // cases used to assert "at least one rung declares deep-research" and "at
    // least one does NOT". Both were claims about the LADDER rather than the
    // derivation — false in a fork below rung 3, where everything works — and
    // restating them as equivalences with the manifest made them TAUTOLOGIES:
    // `topologiesFor` reads RUNG_BY_ID and the expectation read RUNGS, both
    // generated from rungs.json, so they passed with deep-research stripped from
    // every rung (measured, not assumed). They are deleted rather than fixed,
    // because this literal already catches both things they protected:
    //   deep-research removed everywhere  -> this case fails (verified)
    //   every topology handed to everyone -> this case fails (verified)
    // A redundant green reads as coverage whether or not it has any.
    //
