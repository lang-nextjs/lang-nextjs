"""The deepagents rung gates react on THIS plane too (#332 steps C4/C5).

    The mirror of apps/fastapi-backend/tests/test_approval_deepagents.py. Parity holds
    the two planes' code and declarations identical; it cannot hold their BEHAVIOUR
    identical, and two planes wired the same wrong way are green under it.
...
def _post(*, topology="react", policy=True, session=True):
    ...
    if policy:
        body.update(POLICY)
    if session:
        body["sessionId"] = "sess-da-django"

    async def go():
        client = AsyncClient()
        res = await client.post(
            "/api/chat/stream/deepagents/",
...
def test_an_ALLOWLISTED_tool_is_NOT_gated_and_RUNS(monkeypatch):
    ...
    assert not _parts(text, "data-approval-pause"), (
