...
async def guarded_stream(agen):
    ...
    THE BACKEND KNEW THE REASON AND THREW IT AWAY. #247 fixed this on the
    fastapi plane; this plane is a separate implementation and kept the
    defect. It surfaced the first time the live-transport job ran for real:
    two cells failed with `upstream_disconnect`, "upstream backend
    disconnected mid-stream", when the actual fault was upstream of the
    proxy and this process was the only layer still holding it.

    Originally reported in #247: chat
    ...
    The proxy was not wrong. `StreamingHttpResponse` has already flushed 200 by the
