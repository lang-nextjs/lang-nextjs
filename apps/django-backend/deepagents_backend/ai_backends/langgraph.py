...
def _build_plan_execute_graph():
    ...
    planner = (planner_prompt | llm.with_structured_output(_Plan)).with_config(
        {"run_name": "django-langgraph-plan-execute-planner"}
    )
    ...
    executor = create_react_agent(
        ...
        tools=TOOLS,
        prompt=SYSTEM_PROMPT,
        name="django-langgraph-plan-execute-executor",
    )
    ...
    ).with_config({"run_name": "django-langgraph-plan-execute-replanner"})
    ...
    return workflow.compile().with_config({"run_name": "django-langgraph-plan-execute"})
