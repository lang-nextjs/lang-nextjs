...
def test_an_INJECTED_saver_is_the_one_the_graph_USES(scripted, client, monkeypatch):
    ...
    res = _post(client)
    assert res.status_code == 200 and "data-error" not in res.text, res.text
...
def test_the_DEFAULT_path_still_gates_when_nothing_is_injected(scripted, client):
    ...
    assert "approval_pending" in res.text, (
...
def test_each_SCOPE_gets_its_own_saver(monkeypatch):
    ...
    """
    monkeypatch.setattr(_common, "_SAVERS", {})
    a = _common.approval_saver("ai_backends.langchain")
    b = _common.approval_saver("ai_backends.langgraph")
    ...
    assert _common.approval_saver("ai_backends.langchain") is a, (
...
def test_injecting_DISCARDS_savers_already_built(monkeypatch):
    ...
    monkeypatch.setattr(_common, "_SAVERS", {})
    first = _common.approval_saver("ai_backends.langchain")

    monkeypatch.setattr(_common, "_SAVER_FACTORY", CountingSaver)
    _common.set_approval_saver_factory(CountingSaver)

    rebuilt = _common.approval_saver("ai_backends.langchain")
