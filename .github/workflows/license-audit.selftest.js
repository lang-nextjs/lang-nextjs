...
const PERMISSIVE_BASE = {
  ...
  'LGPL-3.0-or-later': [{ name: '@img/sharp-libvips-linux-x64', versions: ['1.2.4'] }],
};
...
check('ACCEPT: permissive tree with the theme exception in place', () => {
  ...
  assert.strictEqual(r.ok, true, `expected pass, got bad=${JSON.stringify(r.bad)}`);
  assert.strictEqual(r.applied.length, 1);
  assert.strictEqual(r.applied[0].name, '@digitalfrontier/theme');
});

// THE ONE THAT MATTERS.
check('REJECT: a DIFFERENT unlicensed dep still fails (not a blanket allowlist)', () => {
  const r = audit({
    ...PERMISSIVE_BASE,
    UNLICENSED: [THEME, { name: 'sneaky-unlicensed-pkg', versions: ['0.1.0'] }],
  });
  assert.strictEqual(r.ok, false, 'gate passed a second UNLICENSED package — exception is a blanket allowlist');
  assert.deepStrictEqual(r.bad.map((b) => b.name), ['sneaky-unlicensed-pkg']);
});

check('REJECT: removing the exception makes the theme fail again', () => {
  const r = audit({ ...PERMISSIVE_BASE, UNLICENSED: [THEME] }, {});
  assert.strictEqual(r.ok, false, 'theme passed with NO exception — something else is doing the work');
  assert.deepStrictEqual(r.bad.map((b) => b.name), ['@digitalfrontier/theme']);
});

check('REJECT: the exception is license-bound, not a blanket pass for the package', () => {
  const r = audit({ ...PERMISSIVE_BASE, 'GPL-3.0-only': [THEME] });
  assert.strictEqual(r.ok, false, 'theme exception leaked to a different license');
  assert.deepStrictEqual(r.bad.map((b) => b.license), ['GPL-3.0-only']);
});

check('REJECT: an ordinary copyleft dep fails', () => {
  const r = audit({ ...PERMISSIVE_BASE, 'GPL-3.0-only': [{ name: 'gpl-pkg', versions: ['2.0.0'] }] });
  assert.strictEqual(r.ok, false);
});

check('REJECT: an SPDX expression with one non-permissive component fails', () => {
  const r = audit({ ...PERMISSIVE_BASE, '(MIT OR GPL-3.0-only)': [{ name: 'mixed', versions: ['1.0.0'] }] });
  assert.strictEqual(r.ok, false, 'SPDX splitter allowed a non-permissive component');
});
