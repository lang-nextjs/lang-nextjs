...
- Rung 3's problems — sub-agent supervision, filesystem state, plan invalidation —
  ...
  to _reconstruct after a reconnect_.
...
stream closes. The request _is_ the unit of work. The client holds the state. If
the tab closes, the work is gone and nobody minds, because the work was seconds long.

In rungs 4–5, the user _fires a run_ and then _reconnects to it_. The run is the
...
| **Reconnection**   | Not meaningful — resend the message        | Load-bearing — you _must_ be able to re-attach     |
...
You can see this already in the repo: `apps/open-swe/components/shell/AppSidebar.tsx` splits
the app by _interaction shape_ — 💬 Live Chat vs ⚙ Queue — not by framework name.
...
→ **Rung 2, [`langgraph`](./2-langgraph.md).** The moment you want _explicit_ state
...
_does_ exist is the substrate it will need — sandbox providers under
...
| `deepagents`         | `react`, `plan-execute`, `deep-research` | `react`, `plan-execute`, `deep-research` |

✅ **The two Python backends now serve the same three topologies.** This block used
to warn that "`deep-research` exists in FastAPI only" and that Django lacked
`web_search` and `ddgs`. Django has had all three since it gained them; the warning
outlived the gap and was telling readers to avoid Django for a reason that had been
deliberately removed (#364).

⚠️ **The TypeScript plane is the one that diverges.** `apps/node-backend` serves
`react` and `plan-execute` for every rung it implements and does **not** serve
`deep-research`, because that topology needs a web-search tool and `ddgs` has no
JS equivalent (#354). If you eject to a fork that keeps just the TypeScript
plane, that topology does not come with you.
...
what a rung _is_, `RUNGS.md` wins — it is the mechanical contract.

_These pages describe a moving repo. The claims above were re-verified against
`origin/main` after #37, #49 and #62 landed; the first version of this page was written
before #37 and shipped describing a rung-4 failure that had already been fixed._
