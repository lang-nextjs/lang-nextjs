...
const sse = (...frames: unknown[]) =>
  frames.map((f) => `data: ${typeof f === "string" ? f : JSON.stringify(f)}`).join("\n\n") + "\n\ndata: [DONE]\n";

const CALL = { type: "tool-input-available", toolCallId: "t1", toolName: "get_counter", input: {} };
const RESULT = { type: "tool-output-available", toolCallId: "t1", toolName: "get_counter", output: { value: 4 } };
...
describe("readTurn / classifyTurn — naming which outcome a turn was", () => {
  ...
  it("a genuinely wrong number is reported as one", () => {
    const v = classifyTurn(readTurn(sse(CALL, RESULT, text("The counter is 7."), finish("stop"))), OPTS);
    expect(v.outcome).toBe("wrong_number");
  });

  it("the healthy turn is ok", () => {
    const v = classifyTurn(readTurn(sse(CALL, RESULT, text("4"), finish("stop"))), OPTS);
  ...
  it("a swallowed upstream error is named as one, not as a missing tool call", () => {
    ...
    const err = {
      type: "data-error",
      data: { id: "stream-error", seq: 0, code: "backend_error", message: "Service temporarily overloaded", retryable: false, origin: "provider" },
    };
  ...
  it("answering from context without calling the tool is its own outcome", () => {
    const v = classifyTurn(readTurn(sse(text("The counter is 4."), finish("stop"))), OPTS);
  ...
  it("nothing is silently dropped: unknown types are counted, bad payloads are counted", () => {
    const t = readTurn(sse(CALL, { type: "data-something-new", data: {} }, "{not json", finish("stop")));
  ...
  it("describeTurn prints the turn, so a failure is readable without re-running", () => {
    const d = describeTurn(readTurn(sse(CALL, RESULT, text("hi"), finish("stop"))));
    for (const field of ["frames", "frame types", "tool calls", "tool results", "finish reason", "errors", "text"]) {
  ...
  it("MUTATION: holding the frames fixed and varying only the text moves the verdict", () => {
     ...
     */
    const verdicts = ["", "7", "4"].map((s) =>
      classifyTurn(readTurn(sse(CALL, RESULT, ...(s ? [text(s)] : []), finish("stop"))), OPTS).outcome
    );
