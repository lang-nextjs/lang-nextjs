...
const MESSAGE_FRAME = makeFrame(
  JSON.stringify({ type: 'chunk', content: 'Hello world this is a test message' })
);
const TOOL_CALL_FRAME = makeFrame(
  JSON.stringify({ type: 'tool_call', toolCallId: 'tc_456', toolName: 'get_weather', input: {} })
);
...
describe('stripMessageIdTransform', () => {
  ...
  bench('transform mixed frame sequence', () => {
    const frames = [MESSAGE_FRAME, TOOL_CALL_FRAME, FINISH_FRAME_WITH_ID, FINISH_FRAME_NO_ID];
    for (let i = 0; i < 250; i++) {
