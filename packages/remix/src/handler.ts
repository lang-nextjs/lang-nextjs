 ...
 *
 * Usage (two-line consumer setup in an action function):
 ...
 *
 * Returns a standard Web API Response. Requires Node.js runtime.
...
/**
 * Creates a Remix action handler that proxies SSE streams
 * from a DeepAgents backend with configurable transforms.
 *
 * The Remix handler is a clean proxy — it applies NO default adapter transforms
...
export function createDeepAgentsHandler(
  ...
  // No default adapter for Remix — consumer provides adapter explicitly
  ...
  return async function handler(args: ActionFunctionArgs): Promise<Response> {
    ...
    // options.maxBodyBytes; 0 / negative disables.
    const maxBodyBytes = options.maxBodyBytes ?? 1_048_576;
    if (maxBodyBytes > 0) {
      const contentLength = args.request.headers.get("content-length");
      if (contentLength) {
    ...
    for (const headerName of STRICT_SINGLE_VALUE_HEADERS) {
      const value = args.request.headers.get(headerName);
    ...
    if (options.getToken !== undefined) {
      ...
      const token = await options.getToken(args);
      if (token) {
    ...
    try {
      ...
      console.error("[deepagents/remix] backend fetch failed", err);
    ...
    const transformedStream = new ReadableStream<Uint8Array>({
      async start(controller) {
        ...
        try {
            ...
            if (done) {
              ...
              for (const frame of accumulator.flush()) {
                if (isFrameOversized(frame)) {
                  console.error(
                    `[deepagents/remix] oversized frame (${frame.length} bytes), skipping`
                  );
                  continue;
                }
          ...
          console.error("[deepagents/remix] mid-stream error", err);
