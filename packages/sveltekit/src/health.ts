...
export async function createReadinessProbe(
  ...
  // Draining signal must NEVER throw — if the consumer's isDraining callback
  // throws (e.g. config-store read error), the probe degrades to
  // ready:false / status:'error' rather than rejecting the returned promise.
  // A rejecting promise would force SvelteKit's request handler to 500
  // instead of returning a clean readiness body, breaking the
  ...
  try {
    ...
    console.error("[deepagents/sveltekit] isDraining threw", err);
